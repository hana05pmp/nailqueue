    ...
    <div>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.slice(0, 6).map((svc) => <Card key={svc.id} className="border-border/70 shadow-soft"><CardContent className="p-6"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-semibold">{svc.name}</h3><span className="font-display font-semibold text-primary">${svc.price}</span></div><p className="mt-2 min-h-10 text-sm text-muted-foreground">{svc.description}</p><p className="mt-4 flex items-center gap-2 text-xs text-muted-foreground"><Clock className="size-3.5" /> {svc.durationMin} minutes</p><Link to="/join" search={{ service: svc.id }} className={buttonVariants({ className: "mt-4 w-full" })}>Choose this service</Link></CardContent></Card>)}
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6"><h2 className="text-2xl font-semibold sm:text-3xl">Today at the salon</h2><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><StatCard label="Total tickets" value={stats.total} icon={ListOrdered} /><StatCard label="Waiting now" value={stats.waiting} icon={Users} /><StatCard label="Served" value={stats.served} icon={Sparkles} /><StatCard label="Avg. service" value={`${stats.avgServiceMin} min`} icon={Clock} /></div></section>
    </div>
  );
}
